...
import api, { API_GET_TOKEN, API_POST_MENTEES, API_POST_MENTORS } from '@/lib/api'
...
export default function SignUpPage() {
  ...
  const validateForm = () => {
    ...
    if (formData.role === 'mentor') {
      ...
      // require proof document for mentors
      if (!proofFile) {
  ...
  // use centralized formatApiError from lib/formatApiError for consistent messages

  function displayError(err: any) {
    if (!err) return ''
    if (typeof err === 'string') return err
    if (Array.isArray(err)) return err.map((e: any) => (typeof e === 'string' ? e : e.msg || JSON.stringify(e))).join(' ')
    if (typeof err === 'object') return err.msg || JSON.stringify(err)
    return String(err)
  }

  // Register mentee API call using multipart/form-data
  async function menteeRegister(data: RegisterData) {
    const payload = new FormData();
    payload.append('user_name', data.username);
    payload.append('password', data.password);
    payload.append('FName', data.firstName);
    payload.append('LName', data.lastName);
    payload.append('Email', data.email);
    payload.append('Phone', data.phone);
    // If backend expects more fields, add them here (e.g., dob, brief)
    if (data.dob) payload.append('dateofbirth', data.dob);
    if (data.brief) payload.append('bio', data.brief);
    // (uses centralized formatApiError imported at module top)

    try {
      const response = await api.post(API_POST_MENTEES, payload, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        const apiErr = response.data?.error || response.data?.detail
        return { success: false, error: formatApiError(apiErr) };
      }
    } catch (error: any) {
  ...
  // Register mentor API call using multipart/form-data
  async function mentorRegister(data: RegisterData) {
    const payload = new FormData();
    // Note: backend field names expected for mentors may differ; adapt if needed
    payload.append('user_name', data.username);
    payload.append('password', data.password);
    payload.append('fname', data.firstName);
    payload.append('lname', data.lastName);
    ...
    payload.append('phone', data.phone);
    // Mentor specific
    if (data.brief) payload.append('brief', data.brief);
    if (data.dob) payload.append('dob', data.dob);
  // Attach proof document if available
  if (proofFile) payload.append('Documents', proofFile);
    // Include role flag for clarity
    payload.append('role', 'mentor');
    try {
      const response = await api.post(API_POST_MENTORS, payload, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setError('');
    // Build and send role-specific payloads
    try {
      let result;
      if (selectedRole === 'mentor') {
        result = await mentorRegister(formData);
      } else {
        // default to mentee
        result = await menteeRegister(formData);
      }

      if (result.success) {
        // Attempt automatic login using the same credentials
        try {
          const params = new URLSearchParams()
          params.append('username', formData.username)
          params.append('password', formData.password)

          const loginRes = await api.post(API_GET_TOKEN, params.toString(), {
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
          })

          // Normalize token response (may be under `data` or root)
          const tokenResp = loginRes.data?.data ? loginRes.data.data : loginRes.data

          if (tokenResp && (tokenResp.access_token || tokenResp.token || tokenResp.refresh_token)) {
            // Persist tokens (cookie + localStorage) via auth store
            useAuthStore.getState().setTokens(tokenResp)

            // Load and persist the user via owner-aware fetch
            try {
              await useAuthStore.getState().fetchAndSetUser(formData.username, formData.password)
            } catch (e) {
              // swallow - fetchAndSetUser handles errors
            }
          }
        } catch (loginErr: any) {
        ...
        // ensure error is a string and pass through friendly formatting
        const errStr = typeof result.error === 'string' ? result.error : JSON.stringify(result.error)
        setError(errStr || 'Registration failed. Please try again.');
      }
    } catch (err: any) {
            ...
            {currentStep === 'form' && selectedRole && (
              <div className="animate-fade-in-up">
                ...
                {error && (
                  <div className="mb-6 sm:mb-8 p-4 bg-red-50 border border-red-200 rounded-2xl max-w-2xl mx-auto">
                    <p className="text-sm text-red-700 text-center">{displayError(error)}</p>
                  </div>
                )}
              ...
              {selectedRole === 'mentor' && (
                <>
                  ...
                  {/* Proof Document Field (required for mentors) */}
                  <div className="sm:col-span-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        required
                        className="block w-full pl-12 pr-4 py-4 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 text-sm text-gray-700"
                      />
                    </div>
                    {proofFile && (
                      <p className="mt-2 text-sm text-gray-600">Selected file: {proofFile.name}</p>
                    )}
                  </div>
                </>
              )}
