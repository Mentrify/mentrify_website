...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        // The token endpoint expects form-urlencoded fields named 'username' and 'password'.
        const params = new URLSearchParams();
        params.append("username", formData.username);
        params.append("password", formData.password);

        const res = await api.post(API_GET_TOKEN, params.toString(), {
          headers: { "Content-Type": "application/x-www-form-urlencoded" },
        });

        // The backend may return the token and related fields either at root
        // or under a `data` wrapper. Normalize and store the entire token
        // response in the auth store, then fetch the user according to owner.
        const tokenResp = res.data?.data ? res.data.data : res.data

        if (tokenResp && (tokenResp.access_token || tokenResp.token || tokenResp.refresh_token)) {
          // Persist tokens and cookie via the auth store helper
          useAuthStore.getState().setTokens(tokenResp)

          // Load the user record using the owner info (mentor/mentee)
          try {
            await useAuthStore.getState().fetchAndSetUser(formData.username, formData.password)
          } catch (e) {
            // ignore - fetchAndSetUser will swallow most errors
          }
        ...
        // Use centralized formatter for user-friendly messages
  ...
  return (
    <div className="min-h-screen bg-white flex mt-[50px]">
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600 text-lg leading-relaxed">
              Simplify your learning journey and boost your productivity with{" "}
              <span className="apple-gradient-text font-semibold">
                Mentrify
              </span>
          ...
          <div className="mt-8 text-center">
            <p className="text-gray-600">
              ...
              <Link
                href="/signup"
                className="text-[#292A36] hover:text-[#292A36]/40 font-medium transition-colors"
              >
                Register now
              </Link>
            </p>
